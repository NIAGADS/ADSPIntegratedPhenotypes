...
# This script is designed to provide users with an output file containing a list of genetically unique samples with harmonized phenotypes for each sample.
# By default, the script will use the most current list of released ADSP WGS files (ng00067.v19), filter the sample list
# to include the recommended list of genetically unique samples in the WGS 58k, and pull phenotype values for each sample.
...
# Default code uses the most recent files for the 58k (ng00067.v19). It also uses the ALL consent version of each file. If you are not approved for all consents, merge the by-consent files for each file type and replace the file names below with your merged file.
...
cc_pheno_file = "data/ADSPCaseControlPhenotypes_DS_2024.11.22.v2_ALL.csv"
fam_pheno_file = "data/ADSPFamilyBasedPhenotypes_DS_2024.11.22.v2_ALL.csv"
adni_pheno_file = "data/ADNIPhenotypes_DS_2024.11.22.v2_ALL.csv"

sample_manifest_file = "data/SampleManifest_DS_2024.11.22_ALL.csv"

ibd_pair_rec_file = "data/gcad.r5.wgs.58506.2025.07.31.pairwise_IBD.summary.xlsx"
...
ibd_keep_drop = read_excel(ibd_pair_rec_file, sheet="R5WGS_recommendation_list", col_names=TRUE, col_types="text")
...
# other_diagnosis_flag: 0/1 indicator for if a subject has an "other diagnosis" (not case or control)
...
####################
# 2. Drop the phenotype replicates (who have phenotypes in both family and case-control studies) 
#    Rule: Keep the one in family study

n=length(intersect(d.cc$SUBJID, d.fs$SUBJID))
print(paste(c("Step 2: subjects in both cc and fam files to drop: ",n),collapse=""))
...
# This section is optional - use if you want only one sample per family (otherwise, remove all of step 3)
# It groups data by family, cases first, and selects the first case in each family to include
# Note: this part of the code does not remove all related individuals, it just keeps 1 member of each reported family that are in the family-based phenotypes. If you need to only keep unrelated individuals, please review the full IBD file and review related pairs.
...
# This code will create a table with sample information for the WGS genetically unique samples and include harmonized phenotype data for each sample
# Note this table will not include PSP/CBD participants (combines case/control, family-based, and ADNI phenotype files)
...
# For definitions of each variable, please see the data dictionary for the output file "ADSPIntegratedPhenotypes_DD_2025.07.31.xlsx"
...
write.csv(sample_pheno_output, file="ADSPIntegratedPhenotypes_DS_2025.07.31.csv", row.names = FALSE)
