...
The default file list that will be utilized is (released in ng00067.v19): 

data/ADSPCaseControlPhenotypes_DS_2024.11.22.v2_ALL.csv

data/ADSPFamilyBasedPhenotypes_DS_2024.11.22.v2_ALL.csv

data/ADNIPhenotypes_DS_2024.11.22.v2_ALL.csv

data/SampleManifest_DS_2024.11.22_ALL.csv

data/gcad.r5.wgs.58506.2025.07.31.pairwise_IBD.summary.xlsx
...
Reads the phenotype files (case/control, family-based, and ADNI), the sample to subject mapping file (Sample Manifest), and the IBD Summary R5 Recommendation list into tables. 

Harmonizes the phenotypes across all files. Adds the following harmonized phenotype values to each set of phenotypes: 
	
 	DX_harmonized (Diagnosis): 0=Control, 1=AD Case 
 	
  	Age_harmonized: AD cases use onset age and controls use age at last visit 
  	
   	other_diagnosis_flag: indicates if a subject has an "other diagnosis" (if the subject is not considered a case or control but does have a diagnosis).
...
The "Family filter" step (Step 3) is optional and can be removed if one sample per family is not required. 
	If used, this section groups the data by families and selects one AD case subject per family to include in the sample list. Please note that this part of the code does not remove all related individuals, it just keeps 1 member of each reported family that are in the family-based phenotypes. If you need to only keep unrelated individuals, please review the full IBD file and review related pairs.
...
Selects the desired output columns and writes the final table to a file named "ADSPIntegratedPhenotypes_DS_2025.07.31.csv" in comma-separated values (CSV) format. 

 
## Variables in Output File (ADSPIntegratedPhenotypes_DS_2025.07.31.csv)
...
Study_DSS: The DSS accession number for the study in which the sample is included 
